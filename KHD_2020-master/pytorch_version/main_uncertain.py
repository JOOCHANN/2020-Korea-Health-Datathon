...
def train(model, criterion, optimizer, batch_loader, epoch):
    ...
    for i, (images, labels) in enumerate(batch_loader):
        ...
        #aleatoric uncertainty apply
        NUM_SAMPLES= 10        
        mu, sigma = outputs.split(2,1)
        prob_total = torch.zeros((NUM_SAMPLES, mu.size(0), mu.size(1))).cuda()
        ...
        outputs = torch.mean(prob_total, 0)
...
def test(model, testloader):
    ...
    with torch.no_grad(): 
        for i, (data, labels) in enumerate(testloader):
            ...
            #aleatoric uncertainty applying
            outputs,_ = outputs.split(2,1)            
